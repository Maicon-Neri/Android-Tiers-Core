...
namespace MOARANDROIDS
{
    public static class Utils
    {
        ...
        public static void ShowFloatMenuSkyCloudCores(Action<Thing> onClick, Thing self = null)
        {
            ...
            {
                CompSkyCloudCore tempComp = Utils.getCachedCSC(core);
                string faccao = (core.Faction != null) ? core.Faction.Name : "";

                Log.Message($"[DEBUG CORE] Encontrado: {tempComp?.getName()} | Facção Desta Estrutura: {faccao} | É um Androide(M8)?: {core is Pawn}");
                if (core == self)
                    continue;
