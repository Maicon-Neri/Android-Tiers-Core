...
namespace MOARANDROIDS
{
    public class GC_ATPP : GameComponent
    {
        ...
        private void refreshSkyMindNetworkState()
        {
            ...
            foreach (Thing thing in connectedThing.ToList())
            {
                if (!canUsePlayerSkyMindNetwork(thing))
                    disconnectUser(thing);
            }
        }

        private bool canUsePlayerSkyMindNetwork(Thing thing)
        {
            return thing != null && !thing.Destroyed && thing.Faction == Faction.OfPlayer;
        }
