...
namespace MOARANDROIDS
{
    public class CompSurrogateOwner : ThingComp
    {
        ...
        private static bool WorkIsActiveSafe(Pawn pawn, WorkTypeDef workType)
        {
            ...
            try
            {
                ...
                return pawn.workSettings.WorkIsActive(workType);
            }
            catch (Exception ex)
            {
                Log.Message("[ATPP] CompSurrogateOwner.WorkIsActiveSafe " + ex.Message + " " + ex.StackTrace);
                return false;
            }
        }
        ...
        public override IEnumerable<Gizmo> CompGetGizmosExtra()
        {
            ...
            if (!isPrisonner && !(currentCAS != null && currentCAS.isSurrogate && currentCAS.surrogateController == null) && WorkIsActiveSafe(currentPawn, Utils.WorkTypeDefSmithing) && Settings.androidsCanOnlyBeHealedByCrafter)
            {
            ...
            if (Utils.GCATPP.isThereSkyCloudCore())
            {
                ...
                // Transfira para o SkyCloud se o SkyCloudCore estiver instalado.
                if (!Utils.GCATPP.isConnectedToSkyMind(currentPawn))
                    yield return new Command_Action
                    {
                        icon = selTex,
